...
func (r *BookingRepo) CompleteBooking(ctx context.Context, userID int64, timeSlot string, name string, phone string) (*domain.Booking, error) {
	...
	// 2. Проверяем, не занял ли кто-то этот стол на это же время (ищем среди подтвержденных)
	var conflictID int
	err = r.db.Conn.QueryRow(ctx, `
	...
	// 3. Стол свободен. Обновляем статус черновика на confirmed
	var b domain.Booking
	err = r.db.Conn.QueryRow(ctx, `
        UPDATE bookings 
        SET time_slot = $1, status = 'confirmed', created_at = $2 
        WHERE user_id = $3 AND status = 'draft' 
        RETURNING user_id, zone, table_name, time_slot, created_at`,
		timeSlot, time.Now(), userID,
	).Scan(&b.UserID, &b.Zone, &b.Table, &b.TimeSlot, &b.CreatedAt)
...
func (r *BookingRepo) GetByUserID(ctx context.Context, userID int64) (*domain.Booking, error) {
	...
	err := r.db.Conn.QueryRow(ctx, `
        SELECT user_id, zone, table_name, time_slot, created_at 
        FROM bookings 
...
func (r *BookingRepo) GetAllActive(ctx context.Context) ([]domain.Booking, error) {
	...
	for rows.Next() {
		var b domain.Booking
		if err := rows.Scan(&b.UserID, &b.Zone, &b.Table, &b.TimeSlot, &b.CreatedAt); err != nil {
			return nil, err
		}
